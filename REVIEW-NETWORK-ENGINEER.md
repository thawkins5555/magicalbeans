...
2. **It sees only IF-MIB on non-Linux gear.** CPU and memory come from
   ...
   *(4.39.0 ships 35 built-in rules and all seven of those are live; see §9
   A-F1 and P-S9.)*
...
### 3.5 After the fixes — the same campaign re-run on 4.39.0
...
`CHANGELOG.md` 4.39.0 Documentation block lists them one by one.
...
The last column records what 4.39.0 did with each row. Thirty-two of the
...
| id | Where | Sev | Tag | What | Owner | Fixed in 4.39.0 |
|---|---|---|---|---|---|---|
