...
| The optional ConfigRX SSH account (ConfigRX → Settings), used only when a device has no SSH credential of its own, from 5.63.0 | Encrypted in `configrx.db`, in its own single row, if you chose to store one | No — same guarantee as the DHCP credential (DPAPI on Windows, the portable secret store elsewhere — see §10). |
