...
| An optional global ConfigRX SSH account (ConfigRX → Settings), used only when a device has no SSH credential of its own, from 5.63.0 | Encrypted in `configrx.db`, in its own single row, if you chose to store one | No — same guarantee as the DHCP credential (DPAPI on Windows, the portable secret store elsewhere — see §10). |
| An optional per-account SSH login (Account page), used by the SSH button and never by ConfigRX, from 5.63.0 | Encrypted in `app.db`, one row per account, if you chose to store one — either from the Account page or by ticking Remember in the terminal's sign-in prompt | No — same guarantee as the DHCP credential (DPAPI on Windows, the portable secret store elsewhere — see §10). |
