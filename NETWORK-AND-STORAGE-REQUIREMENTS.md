...
| Nodes SNMP polling | UDP | 161 (fixed) | GET/GETBULK to each configured device, on its own poll interval. On a Cisco device (enterprise arc 9), from 5.32.0, this includes thirteen extra column walks for CISCO-STACKWISE-MIB stack power (five port columns, four per-switch power columns, four per-stack columns) — read on every poll once a switch has answered any of them, and retried at most once an hour on a Cisco device that never does |
