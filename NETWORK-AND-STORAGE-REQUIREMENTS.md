...
| Nodes copper/laser transceiver proof (MAU-MIB), from 5.25.0 | UDP | 161 (fixed) | One extra column walk (`ifMauType`, one row per port) alongside the existing five-minute environment poll, and only on a device the entity-sensor walk has already mapped to at least one port — a device with no transceivers at all is never asked. A switch that does not answer the MIB costs one failed GET an hour instead, not one every environment poll |
...
**File modes, on POSIX hosts.** From 4.39.0 the data folder is created `0700`
