...
# ---------------------------------------------------------------------------
# "Best-known display name for an IP", shared so Syslog's Host column, the
# Alerts module's Object column and NetPath's hop labels agree with the Nodes
# module's own display precedence rather than each picking a different one.
# ---------------------------------------------------------------------------
...
def resolve_names(nodes_db, app_db, ips) -> dict:
    """resolve_name batched into one devices_by_addresses and one hostnames
    query: {ip: name}, leaving out addresses that resolve only to themselves."""
