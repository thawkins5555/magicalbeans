...
ENT_PHYSICAL_SOFTWARE_REV_FIRST = ENT_PHYSICAL_SOFTWARE_REV + ".1"  # index 1: chassis
ENT_PHYSICAL_FIRMWARE_REV = "1.3.6.1.2.1.47.1.1.1.1.9"    # ENTITY-MIB, per entity
ENT_PHYSICAL_FIRMWARE_REV_FIRST = ENT_PHYSICAL_FIRMWARE_REV + ".1"  # index 1: chassis
CISCO_SYS_CONFIG_NAME = "1.3.6.1.4.1.9.2.1.73.0"  # OLD-CISCO-SYS-MIB sysConfigName: boot image file path
HR_SW_INSTALLED_NAME_FIRST = "1.3.6.1.2.1.25.6.3.1.2.1"  # hrSWInstalledName.1
WLSX_SYS_EXT_SW_VERSION = "1.3.6.1.4.1.14823.2.2.1.2.1.28.0"  # WLSX-SYSTEMEXT-MIB wlsxSysExtSwVersion

SW_VERSION_OIDS = {  # (version OID | None, image OID | None) per arc; None = read from sysDescr
    12356: ("1.3.6.1.4.1.12356.101.4.1.1.0", None),                          # fgSysVersion
    2636: (None, HR_SW_INSTALLED_NAME_FIRST),
    11: ("1.3.6.1.4.1.11.2.14.11.5.1.1.3.0", None),                          # hpSwitchOsVersion
    14988: ("1.3.6.1.4.1.14988.1.1.4.4.0", "1.3.6.1.4.1.14988.1.1.7.4.0"),   # mtxrLicVersion; mtxrFirmwareVersion
    25461: ("1.3.6.1.4.1.25461.2.1.2.1.1.0", None),                         # panSysSwVersion
    1916: ("1.3.6.1.4.1.1916.1.1.1.13.0", None),                            # extremePrimarySoftwareRev
    674: ("1.3.6.1.4.1.674.10895.3000.1.2.100.4.0", None),                  # productIdentificationVersion
    1991: ("1.3.6.1.4.1.1991.1.1.2.1.11.0", None),                          # snAgImgVer
    ...
    2604: ("1.3.6.1.4.1.2604.5.1.1.3.0", None),                             # sfosDeviceFWVersion
    3375: ("1.3.6.1.4.1.3375.2.1.4.2.0", None),                             # sysProductVersion
    5951: ("1.3.6.1.4.1.5951.4.1.1.1.0", None),                             # sysBuildVersion
    890: ("1.3.6.1.4.1.890.1.15.3.1.6.0", None),                            # sysSwVersionString
    161: ("1.3.6.1.4.1.161.19.3.3.1.25.0", None),                           # swVersion
    26928: ("1.3.6.1.4.1.26928.1.2.12.0", None),                            # ahFirmwareVersion
...
SW_VERSION_OIDS[17713] = SW_VERSION_OIDS[161]  # Cambium's other arc, same swVersion

FW_VERSION_OIDS = {  # arc: firmware/boot-ROM OID, separate from the software version above
    8741: "1.3.6.1.4.1.8741.2.1.1.4.0",   # snwlSysROMVersion
...
WTP_PROFILE_RADIO_ENTRY = f"{WTP_TABLES}.2.1"  # indexed by profile name, joined via fgWcWtpSessionWtpProfileName
...
WTP_SESSION_UPTIME = f"{WTP_SESSION_ENTRY}.8"           # TimeTicks, AP uptime since boot
WTP_SESSION_SESSION_UPTIME = f"{WTP_SESSION_ENTRY}.10"  # TimeTicks, since joined the AC
...
WTP_PROFILE_RADIO_CHANNEL_WIDTH = f"{WTP_PROFILE_RADIO_ENTRY}.20"  # profile-configured width, not running width

CHANNEL_WIDTH = {  # FgWcWtpChannelWidthType
...
# Cisco fan state, arc 9 only: the FRU control table is tried first, the
# classic ENVMON one only as a fallback when it comes back empty. Reuses
# PsuTable, whose shape already fits.
FAN_TABLES = {
    9: (
        PsuTable(   # CISCO-ENTITY-FRU-CONTROL-MIB cefcFanTrayOperStatus
            ...
            state_map={2: 0, 3: 2, 4: 1},   # up/down/warning -> ok/failed/warning
        ),
        PsuTable(   # CISCO-ENVMON-MIB ciscoEnvMonFanState, fallback when the FRU table is empty
            ...
            state_map={1: 0, 2: 1, 3: 2, 4: 2, 6: 2, 5: 3},   # normal/warning/critical.. -> ok/warning/failed/not present
        ),
    ),
}
