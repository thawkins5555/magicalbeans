...
**A link that opens a device's own pane now also makes sure the Devices
grid is actually showing that row, from 5.30.0.** It clears the Find box
and returns the grid to page one; if the row would still be hidden, it also
clears the Profile, Group, Status, Only-offline, Only-in-maintenance and
Only-with-overrides filters. If the fleet runs past one page, the grid
...
- **The device pane has an ADDRESSES subtab** listing every address on
  ...
  last seen, refreshed by the hourly poll. The interface column names
  the actual VLAN or interface the address belongs to (e.g. `Vlan10`,
  `GigabitEthernet1/0/1`), from 5.30.0 — it used to show only the raw
  SNMP index number; it falls back to that number only if the interface
  itself is no longer on file. This is why a trap or a syslog line
  arriving from a loopback is attributed to the right device. A newly
  added or promoted device shows nothing here until its first poll
  fills it in, at most one poll interval away. Its ARP table is a
  separate table, on its own ARP subtab.
...
- **A first notification can wait for the roll-up, from 4.47.0.**
  **Alerts → Settings → Hold the first email for N seconds**
  ...
  loses nothing: what is still due is read back from the database, not
  kept in memory.
...
- **A third channel, alongside email and the webhook.** **Alerts →
  ...
  `[RECOVER]` — with mutes and maintenance windows honoured exactly as
  they are for email; the hourly cap is its own budget, separate from
  email's and the webhook's, and a text suppressed by it is recorded as
  "not sent: over the N/hour text limit" rather than silently dropped.
