...
**A device name is now a way to that device.** Wherever the application names a device — the Object column on the alert list first among them, and also the Syslog and SNMP-trap source, wireless access points and their controllers, IPAM hostnames, the Availability and Top-N reports, ConfigRX and MAPPER's upstream suggestions — the name is a link. Where the row already knows which device it means it opens that device's pane directly; where it knows only a name it opens Nodes with the search box filled in, which lands on the device when the name matches one. The alert list's own detail pane has linked its object since long before this; what changed is that the list agrees with it. Two details worth naming: the search link deliberately does not run the MAC-address search that the identical-looking IPAM link does, because a device whose name happens to be a few hex characters should not be answered with a note about MAC addresses; and an account that cannot read Nodes is shown the name as plain text rather than a link into a tab it would be refused.

**Maintenance Mode.** A device taken out of service by hand, indefinitely: no new alerts and no mail of any kind, including the recovery notice, until a person turns it off. It is the third such mechanism and it removes neither of the others — the 24-hour mute is still there for the alert you are looking at right now, and scheduled maintenance windows are still there for a planned span. What this adds is the case neither covered: a device that is out until further notice. **Polling continues** — graphs, status and metrics stay live, because "we are working on it" is not "we stopped watching it" — and alerts already open stay open and visible. It records who set it, when, and an optional reason, all three shown on the device. The time a device spends in maintenance is subtracted from its downtime in the Availability report, in its own column beside the two that were already there, so it is always recoverable which mechanism excused which minutes. It can be set on one device from its pane in Nodes, on a selection from the bulk bar, and ended from the alert list; devices in maintenance can be filtered for, and their number appears with the other alert counts so one left on and forgotten is visible. Two defects had to be fixed for any of it to hold, and both were older than this release: **a muted device's open alert was still sending reminder mail** every re-notify interval, which is the opposite of what mute is documented to do; and **deleting a device left its alert state behind**, where SQLite reissues the freed row number, so the next device added would silently have inherited the deleted one's suppression.

**The Web button stays inside its tunnel.** Pressing WEB on a device could land you on `https://<this server>/home.asp` — the SappiWhere server's own address, not the device's. Since 5.1 the button deliberately opens a short-lived tunnel on this host rather than pointing your browser at the device, so that it works from anywhere the application is reachable; that part was working as designed. What was not: the tunnel copied bytes without reading them, so when the device built a redirect out of the address the browser had used, dropped the port, and answered "go to /home.asp over there", the browser did exactly that and left the tunnel — arriving at this server, which has no such page. For a device whose interface is plain HTTP the tunnel now reads the headers going past: the device is told its own address on the way out, and its redirects, cookie domains and refresh headers are mapped back onto the tunnel on the way in. Message bodies are still passed through untouched and never held in memory, and anything the parser does not fully understand — a WebSocket upgrade, a header shape it cannot frame — falls back to the old byte-for-byte copy for the rest of that connection, so no device that worked before can break. **This also fixes the absolute-link escape** the button's own help text used to describe as unavoidable. A device configured for HTTPS keeps the blind tunnel, since reading its headers would mean decrypting it here; the dialog now says so rather than leaving it to be discovered.

**SNMP polling to Palo Alto firewalls.** No such firewall was available to test against and the failing device's recorded error could not be retrieved, so rather than guess, four real defects were fixed and the diagnostics were made good enough that the next occurrence explains itself. The one that best fits the report — connectivity and community confirmed, polling still failing — is that **a single slow reply during the interface walk failed the entire poll**, even though the system details had already answered, which is why such a device shows its model and description and still reads as failing. A partial walk now degrades the interface list, which is what it is, instead of condemning the device; a walk that collected nothing at all still counts as a failure. Second: **an error answer other than "too big" ended the walk in silence** — no exception, no log line, nothing recorded — so the device read as healthy with no interfaces and no stated reason. Third: an agent that refuses bulk requests was re-asked on every walk forever, because the fallback was worked out and then not remembered. Fourth: a community string was never trimmed and never split, while discovery does both — so a profile holding two comma-separated communities made discovery succeed and polling fail, silently, because a wrong community is dropped without an answer rather than refused. Commas are now refused on save with a message pointing at the credential alternates that do the job properly, and existing values are reported rather than transmitted. Alongside those, the device **Test** button now runs a real interface walk instead of six scalar reads that pass on every one of these failures, and reports each phase's timing, the bulk size the device actually accepted, and how many replies were rejected rather than missing — which is the difference between a firewall problem and a credential problem.

**Interfaces no longer read in Tbps.** A handful of ports showed a speed a thousand times too large — a 10 Gb/s port reading 10 Tbps. The display was right and the switch was wrong: every interface reports its speed twice, once in bits per second and once in megabits, and some agents fill the second figure in kilobits. Nothing checked. The two readings are now cross-checked: a figure above any speed a real interface can run, or one that disagrees with the other reading by orders of magnitude, is not believed, and where the older reading cannot answer at all — it saturates above about 4.3 Gb/s, which is exactly where these ports live — the suspect figure is retried as kilobits and kept only if that lands somewhere possible. Genuine 400 and 800 Gb/s ports are untouched. **The utilisation figures on those ports come right with it**: they divide by this number, so every affected port had been reading near zero percent no matter how busy it was.

**The dashboard's access note is gone.** Removed as asked. The two messages a tile shows when it has nothing you can read are not this note and are still there — they are what the tile says instead of a number, not a standing caption.

**The IPAM read-only note is gone.** Removed as asked, in both places it appeared: the DHCP page and the add-server dialog. The line about what is stored in a saved credential stays, since that describes the credential and not the feature.

**MAPPER draws one cable once.** A link between two switches could carry its VLAN count and its port names twice, printed over each other. The cause was that a cable seen by both CDP and LLDP became two records: LLDP rows carry what kind of identifier they hold and CDP rows never did, so the two could not be recognised as the same cable, and the map has no way to draw two links between the same pair except exactly on top of one another. A second pass now folds them, keeping the better port name of the two and naming both protocols; a second, rarer duplicate has been closed with it, where one identifier matching several interfaces on a switch — a stack's shared base address, for one — produced one link per match. The status bar's link and VLAN counts, the VLAN table and the CSV export all counted the cable twice as well, and all three come right with the fold.

**Release review.** A Fable review of the combined diff found eight defects, none of them severe, all of which had passed a green 112-suite run; each fix carries a test that fails without it. The one that mattered: on the settings this application ships with, **a short spell in maintenance mode could swallow an alert's only notice for good** — an alert whose first mail was still being held for roll-up, on a device put into maintenance for a two-minute cable move, was recorded as decided and then never sent, because the reminder sweep that was supposed to catch it is off by default. Ending maintenance now re-arms exactly those notices, and one that waited out a long maintenance is still owed rather than written off as backlog. Two more were in this release's own new code: the rewritten neighbour lookup **lost a link entirely** when a disabled device shared a chassis address with a live one — a duplicate somebody kept rather than deleted, or a pair of routers sharing a virtual address — and the new speed guard **divided a large port-channel by a thousand**, which then cost that bundle every rate sample it took. Fixing the second needed a reading the poller had never taken: what kind of interface it is looking at, which is the only thing that distinguishes a genuine 3.2 Tb/s bundle from a port claiming a speed that does not exist. On the web tunnel: rewriting the device's address into the request but leaving the two other headers that name it would have broken login forms on gear that checks they agree, and the lane fixed a fourth such place the review had not spotted — a request written in absolute form; a device redirecting to HTTPS now leaves the tunnel saying so instead of bouncing the browser in a loop; and a reply declaring two conflicting body lengths goes down the untouched path rather than being framed on a guess. The last two were diagnostics turning on themselves: the device **Test** button answered an unexplained server error, rather than the message that names the problem, for the one community-string fault this release added a check for; and a switch with more than 512 interfaces — a Palo Alto with per-VLAN subinterfaces reaches that easily — had the truncation written into its SNMP error line on every poll, where a red line that never clears is how the next real error gets missed. It now reads as a note under the interface list it explains. Three things the review raised were checked and corrected rather than taken on trust: it named the wrong expression for the neighbour defect (the right lines, the wrong half of them), its worked figure for a wrapped 400G port was out by a factor of two, and its suggested fix for the port-channel assumed a reading the poller does not take.

### 5.3.0 — Six asks

Short notes, one per request, in the order they were given.

**Editing a rule saves again.** Pressing Save on Alerts → Rules → **Interface flapping** answered `Failed: Cannot read properties of null`. The two flapping fields were written inside the editor's threshold-only branch but read back outside it, and the flapping rule is an interface-event rule, not a threshold one — so the boxes were never on screen and Save read them anyway. They now sit outside that branch, which also makes them editable at all: before this they could not be reached from the dialog even when it did not crash. The shared field reader behind every dialog in the application now names the field it could not find instead of failing on a null, so the next dialog to grow a conditional box reports which one rather than a message nobody can act on — and deliberately does not substitute a blank, which would have quietly saved a setting nobody touched.

**Severity 1 and 2 stand out on the alert list.** Rows at severity 2 and worse carry a red tint and pulse, about one breath every three seconds — slow on purpose, well under the rate the photosensitive-seizure guidance sets. The scale counts down, so this covers severity 0 as well; it reuses the same floor the desktop-notification setting already uses, so the two cannot drift apart. **The flashing stops when the alert is acknowledged** and the tint stays, so movement on a wall display means nobody has picked it up yet rather than that something is merely bad. A viewer whose system asks for reduced motion gets the tint and no movement at all. The severity word itself is drawn in red, which on a red row fell below the readable-contrast floor in five of the seven palettes at rest and in all of them at the pulse's peak; inside a highlighted row it now reverts to ordinary text, since the row is already carrying the colour.

**The Fleet tile names the devices that are down.** Under the up/down/unknown figures, the Dashboard now lists up to ten down devices by name — the same name precedence the rest of the application uses, not the raw stored one, so a device discovered by address still reads as itself — each linking straight to its device pane, with "and N more" linking to the filtered list when the outage is bigger than ten. It rides the payload the Dashboard already fetches and sits inside the permission the Fleet tile already required, so an account that cannot read Nodes sees the tile exactly as before.

**Reboot alerts read in minutes.** A device that had been up two and a half minutes was described as "uptime dropped from 1036800000 to 15000 hundredths of a second after 300s" — the only place in the application that showed a human a raw SNMP tick count instead of dividing it down first. It now reads "uptime dropped from 120d 00:00:00 to 00:02:30.00 after 5 m 00 s". A second defect went with it, and it is probably the one that was actually being looked at: the reboot email template has always printed **Previous reported uptime** and **Current reported uptime**, and nothing in the application ever filled them in — every real reboot email rendered two blank lines, while the template preview showed plausible figures because the preview supplies its own. Both now carry the real numbers. The figures are read back off the stored event rather than the device, because by the time an alert is raised the device row already holds the post-reboot reading and the earlier one is gone for good.

**Switching NetFlow time windows stops piling up work.** Stepping the range dropdown from 15 minutes to 30 days used to fire thirteen requests covering seven windows, nine of them for windows already abandoned, none of them cancelled, all queued on the same lock the collector writes flows through — so the answer actually wanted arrived last. It now fires two requests for the one window landed on. Four separate causes: only the mouse wheel collapsed a burst of changes, so the dropdown, the zoom and pan buttons, the keyboard and the brush did not; nothing cancelled the request for a window already left, because the cancellation key included the window itself and so never matched; the chart and the record list were fetched one after the other though neither needs the other; and the 2-second refresh tick fetched whatever half-finished window it happened to land on. Opening the tab also fetched twice. Alongside that, **the page now says it is working**: the application's busy indicator was only ever raised by the background poll and never by a direct refresh, so every window change looked frozen — that is fixed for every module, not only NetFlow, and the chart, the top-talkers and the record table each say **Loading…** while the new window is fetched. The trade is that one isolated window change now waits out a quarter-second collapse before asking; on real data that is a small fraction of the query it replaces, and several queries are no longer issued at all.

**Optic power alerts use each transceiver's own published limits.** A 10GBASE-SR receives at −1 to −10 dBm and a ZR at −3 to −24, so the single −22 dBm rule that shipped was either deaf to one or crying wolf about the other. Where a Cisco switch publishes its transceivers' own alarm and warning thresholds, each port is now judged against the numbers for the part actually in the cage, read hourly — a transceiver's limits change only when somebody swaps it — and decoded at that sensor's own scale, which differs between ports on the same switch and is the one mistake that would be wrong by a factor of a thousand while looking entirely plausible. Four rules became eight: receive and transmit, high and low, each with a warning and an alarm level, the alarm suppressing its own warning so one failing optic raises one alert rather than two. The DOM tables now show the limits each port published, and say plainly when a port published none.

**What this narrows, deliberately.** These eight rules now alert *only* where thresholds are published, as instructed: the global default is gone and, for these rules alone, a per-device threshold set by hand is no longer consulted either — though switching a rule off for one device still works. This release reads Cisco's table only, so optics on every other vendor raise no optical power alerts until that vendor's support lands. Optic temperature, voltage and bias are untouched and still run on their shipped numbers, as do the six other threshold rules and their overrides. Two consequences worth knowing: optical power is now judged per port only, so the device-wide form of these alerts is gone; and on a multi-lane optic the high-side rules watch the dimmest lane, so a single hot lane on a breakout can go unseen — the low side, which is the side that predicts a failing link, is unaffected.

**Release review.** A Fable review of the combined diff found three defects, all of which had passed a green 107-suite run; each fix carries a test that fails without it. The one that mattered: an optic power alert already open on a port that publishes no limits would never have been resolved. The evaluator skipped such a port before reaching the code that closes an alert, and threshold rules have no timeout — so on the first upgrade, every optic alert standing on a switch that does not publish thresholds would have stayed open for good, on exactly the gear this release stops alerting for. Two comments and a paragraph of the internals guide claimed the opposite. The evaluator now closes an alert it can no longer judge, and a migration clears the ones standing at upgrade. Also fixed: the second pass that carries an operator's settings onto a new sibling rule ran over every pair rather than the six it was added for, so an operator who had muted the chassis-temperature warning and deliberately kept its critical rule on would have had that reverted; and the reboot note's reader accepted the old 5.2 sentence too, printing the raw tick counts it exists to remove. Alongside those, the NetFlow chart now says when a load failed rather than reading "Loading…" for ever, and four comments were corrected where they described something the code does not do — including one, about which migration order is load-bearing, that was simply not true. Two things the review raised were checked and found not to apply: no cell in the alert list uses a colour that would fall below the contrast floor on the new highlight (the tokens that would are all in the detail pane, outside the table), and a test that failed once in a full run turned out to compare two renderings a second apart rather than to be a symptom of the change — it predates this release and is now pinned against the clock. One trade is worth naming: the upgrade migration cannot see which ports publish limits, since that fact lives in another database, so a still-breaching optic on a switch that does publish has its alert resolved and immediately re-opened, losing an acknowledgement but no alert.

### 5.2.0 — Eight asks

Short notes, one per request, in the order they were given.

**A dark optic no longer alerts.** An SFP reading −40 dBm is a port that is powered off or has no fiber in it, not an optic in trouble — but `sfp_rx_power_low` and `sfp_tx_power_low` are low-side rules, so −40 was past every threshold and every dark port raised a warning. A reading at the bottom of the scale (−40 dBm or below, or a value that is not finite at all — 0 dBm is 1 mW, a nominal transmit level, and is treated as the reading it is) is now recognised as "no light": it never breaches either rule, and it resolves one that is already open — a lit optic that goes dark would otherwise have kept its alert, on a reading fresh enough that nothing ever expired it, and every alert a 5.1 build raised on a dark port is open at upgrade time and would have become permanent. The test is keyed to the two optical-power metric families, so no other `below` rule can inherit it, and it lives in the evaluator rather than at the point the metric is written: a −40 already in the metric history would otherwise have gone on re-evaluating for the fifteen minutes of `threshold_stale_s`, and one written by an older build would never have expired at all. The same reading is now also kept out of a multi-lane optic's worst-of, where a single dark lane used to beat three healthy ones, and the DOM tables in the device and interface dialogs show **No signal** in place of the number, with the raw figure still on hover.

**DOM and SFP are told apart on the interface list.** The badge used to appear only where DOM sensors were found, so a transceiver with no DOM — and an empty cage — looked exactly like a copper port. The environment walk now also reads `entPhysicalClass` and `entPhysicalModelName`, and resolves each cage to its port through the containment chain it already walked for sensors. A port with DOM readings wears **DOM**; a cage whose contents name a transceiver wears **SFP**; an empty cage says so on hover. A cage has to name itself — a form factor (`SFP`, `SFP+`, `QSFP`, `XFP`, `GBIC`) or an optical part number — because platforms that model every front port as a container with a port inside it would otherwise have badged every copper port on the switch; bare `1000BaseT` deliberately does not match. `interfaces.media` carries the three states, and a walk that times out — whether it answered nothing at all or stopped part-way through — still leaves the badges alone rather than stripping them or downgrading an occupied cage to an empty one. Both columns are only read for a device whose entity table maps something to a port, but for one that does they are two more full walks of the `entPhysical` table every five minutes. `entPhysicalVendorType` was read too and no longer is: it is an OBJECT IDENTIFIER column, so what comes back is a dotted number rather than a name, and it never contributed a badge.

**Data & Retention lines up on the left.** Every row in the section is now one grid rather than a grid of its own, so the labels share a column, the inputs share a column, and each readout sits immediately after the box it belongs to — the oldest-record note inline with the file's location, the % used meter inline with the size cap. The old rule gave the input column all the leftover width of the page, which pushed the notes, the meters and even the literal " MB" out to a ragged right edge that moved from row to row; it became visible in 5.1.0 when the oldest-record notes were added to rows that until then had nothing after the input. Four further defects in the same section went with it: an account without Settings read saw `0 B used · NaN%`, because the storage figures are withheld from it but the caps are not; a database over its cap read a flat `100%`, since the share was clamped before the percentage was taken, which is why the Dashboard and Settings disagreed about the same file; the wireless, ConfigRX and map files were counted in the "on disk in total" line but had no row of their own to account for the difference; and the two uncapped files drew a permanently empty meter beside a real byte count. The store list behind all of it is now written once and read by Settings, the Dashboard, the desktop console and the new alert below.

**An alert when a database is running out of room.** Nothing watched storage before. Two rules ship enabled: **A database is close to its size cap**, raised per file at 85% and clearing at 80%, its message saying which of the two bands the file is in once it passes 95%; and **The volume holding the databases is low on free space**, with the warning and critical thresholds settable under Data & Retention. Both are `system` rules — the application reporting on itself, like the existing SMTP and polling-pool rules — so neither is ever emailed, which is deliberate: a full disk is exactly when the mail path is least trustworthy. The alert names the file, its path, its size, its cap and the retention setting that governs it. The clear band sits below the raise band on purpose, because trimming holds a busy file just under its cap and a single threshold would have opened and closed the alert on alternate sweeps; the volume alert clears a margin above its warning threshold rather than at it, for the same reason.

**Re-discover a scan that has finished.** A discovery subnet that has run to completion and not been removed gained a **Re-discover** button beside Remove, on finished, cancelled and errored rows alike. It starts a fresh scan of the same target rather than reopening the old one: the results of a sweep are a record of what was found when, and a job's thread cannot be restarted anyway. So that the repeat matches the original, `discovery_jobs` now stores the polling profile the scan was started with, along with the per-scan timing overrides — but not the community strings those overrides carry, which stay out of the table and are re-derived from the profile on every start, so a community added since is picked up. A rescan is refused while that scan is still running, and while another running job already covers the same target; a row left `running` by a killed process does not wedge the button. Nothing can be duplicated by re-scanning: a device already added comes back marked "Already added" and cannot be ticked, exactly as it does when two sweeps overlap.

**Upstream suggestions moved to MAPPER.** The button was left on the Nodes strip from when the topology graph was a Nodes page; 4.53.0 retired that subtab and 4.54 shipped MAPPER as its replacement, so this finishes the move that release began. It now sits in the Mapper strip between Refresh and Maps, and works with no map open, since the suggestions are fleet-wide. Nothing about the dialog, the matching or the two endpoints changed, and opening it still needs only Nodes read while Apply still needs Nodes write. The per-device upstream field in a device's own Edit dialog is untouched.

**Every address a merged device answers on is searchable.** Merging two devices kept the losing device's address all along — but nothing looked for it: the Nodes search matched only columns of the devices table, so typing the merged-away address found nothing and the address was, to an operator, gone. The search now also matches the addresses a device answers on, so any of them finds the surviving device. The Name / IP cell shows the configured address with a **+N** beside it naming the others, the same way the discovery table marks a box a sweep reached twice, and the devices export gained an addresses column so an exported fleet is not missing them either.

**Release review.** A Fable review of the combined diff found sixteen defects, thirteen of which are fixed here; each fix carries a test that fails without it, because every one of these passed a green suite. The three that mattered: a dark optic stopped *raising* a low-power alert but never *cleared* one, so a lit optic going dark held its alert open on a stale reading — and, worse, every alert 5.1 raised on a dark port was open at upgrade and would have stayed open for good, the exact alerts this release exists to remove; the minute-tier flow compaction rebuilt a fixed twenty-bucket redo window before advancing its watermark, so above roughly 300 flows a second the budget was spent before it reached new buckets and the summaries fell permanently behind, leaving every chart back on the full scan they exist to avoid (it now builds forward first and redoes only the buckets a flush actually touched, which also fixes flows from an exporter more than twenty minutes behind never reaching the summaries at all); and the record list's new scan bound was applied to the time ordering as well as the volume orderings, so any window older than the newest two million flows came back empty where 5.1 returned rows. Also fixed: `oldest_ts()` on the flow store became a full table scan run every ten seconds under the collector's write lock (0.117 s at 2.4M rows, now three index probes and 0 VM steps); the flow size trim ran before the summaries were built and deleted the history they were about to summarise; a partial ENTITY-MIB walk stripped every SFP badge on the device until the next pass, the strip the badge work was meant to prevent; the database-cap alert's 80–95% hysteresis was defeated by its own thirty-minute auto-resolve, and the free-space alert had no clear margin at all and would flap every sweep near its threshold; the free-space thresholds accepted a critical floor above the warning one, which made every volume alert critical and the warning band unreachable; the retention sweep walked the whole flow table when one exporter's clock was far in the past; the residual under a summarised chart could stack negative while two threads rebuilt one bucket; a sampling rewrite marked one shared cursor that the first tier consumed before the second could; and the new rollup retentions were unvalidated, where a negative put the cutoff in the future and quietly took the tier out of service. `entPhysicalVendorType` was dropped from the entity walk entirely: it is an OBJECT IDENTIFIER column that renders as dotted digits, so it could never have matched a transceiver name and cost a full walk per device every five minutes. Three findings were left: making the 95% cap alert genuinely escalate its severity needs a change to `alertengine.py` with reach well past this release, so the message names the band instead; a cage whose port sits under a transceiver module rather than directly inside it still earns no badge; and the fifty-million-flow bench was abandoned mid-backfill, which is the one-time paced build rather than the query path.

**NetFlow answers from summaries instead of scanning every flow.** Every view read the raw flow table: the overview grouped and bucketed the whole window on each refresh, so at the shipped five-million-record cap a thirty-day view scanned five million rows to draw one chart, and retention made it worse by deleting a fortnight of rows in a single statement under the writer's lock — on a UDP collector, where a stalled writer is lost data. Flows are now summarised as they age, into per-minute and per-hour tables holding the heaviest keys of each bucket for each of the eleven group-by dimensions, plus one row per bucket carrying the bucket's true totals. An unfiltered view reads the summaries for the settled part of its window and the raw table only for the last few minutes, stitched at a boundary that cannot double-count; totals stay exact because they come from the per-bucket row rather than from the kept keys, and anything below the cut lands in "other" exactly as it always did. Query cost now follows the width of the window rather than the number of flows, which is the point: at ten million flows a thirty-day view goes from 92.6 seconds to 205 milliseconds, and a three-day view from 8.3 to 0.023, with answers identical to the raw ones in every case the suites compare. Any query carrying a filter still reads raw flows and is exact. Retention was rewritten onto the same chunked delete the traces file already used, so it hands the lock back between batches; summaries keep their own, longer retention (a minute tier for two days, an hourly tier for ninety), so a thirty-day chart now outlives the fourteen days of raw records it was drawn from. On the collection side the writer drains its queue greedily instead of one batch at a time, and the sampling-rate correction — which rewrote a whole run's worth of flow rows under the writer's lock every time an exporter announced a rate — is bounded to the last fifteen minutes. Summarising an existing file is paced a few seconds at a time in the background, newest first, and until it reaches back to a given window that window behaves exactly as it does today, so no view is ever slower than before.
...
**Release review.** A Fable review of the combined diff found and fixed: the new second trim stage deleted rollup hours on every pass, not only once raw samples were at their floor, so a cap raw alone could satisfy cost history (the lane's own test pre-emptied the raw table and could not see it; a test now pins it); the oldest-record query on the flows and traces files was a full scan every ten seconds under the writer's lock (now the first row by insertion order); an ENTITY-MIB walk that timed out mid-way stripped every SFP badge on the device until the next pass; RouterOS 7's dated export header was not on the MikroTik volatile list; the per-port evaluator read a device's interfaces on every tick for an alert that was already open and unchanged; a settings save landing while a sweep was running could report the sweep finished before its own had run; the oldest-record read raised on a store that had just been closed; a browser closing a tunnelled connection to a device that ignores half-close pinned that connection's slot until the idle timeout; and the relay URL was built from the raw Host header without a character allowlist.
...
**Release review.** A Fable review of the combined diff found and fixed: the environment poll walked `entPhysicalName` on every vendor, every cadence, for a name fallback only Cisco needs (now gated on the same Cisco test as the table walk); the "none mapped to an interface" event fired for a UPS or room monitor that maps nothing to a port by design (now only for Cisco gear or an unmapped optic); clearing the new worker field in the Start discovery dialog posted 0 and refused the scan (an empty field now means the default, and the settings pane never stores 0); and the macOS memory readout was 1024 times too large, since `ru_maxrss` is already bytes there. A second pass over MAPPER then found and fixed: a redraw landing mid-drag replaced the element the drag had captured, so its release never arrived and auto-refresh stayed skipped until the next press; a first draw into the 200-pixel fallback box counted as the map's one fit; a release the page never saw (Alt-Tab mid-pan) left a gesture flag set for good; and Space on a focused link neither panned nor followed it. The full-suite gate, run while a browser walk loaded the machine, then caught a race that predates this release: `POST /api/update` started its job thread and only then read the job's state, so a job that failed at once (no network) answered "failed" to the very press that started it; the snapshot is now taken before the thread runs. The browser walk that closes the release then hung on the Debug tab, and the cause predates this release too: sorting any hand-built table (Debug's worker lists, the discovery jobs list) rewrote its sort caret on every pass, which the observer that re-applies a plain table's sort took as a change and answered with another sort, forever, freezing the page. The caret is now written only when it changes. Comments across the release were trimmed to the why.
