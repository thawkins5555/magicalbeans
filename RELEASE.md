...
1. `GET /repos/<owner>/<repo>/tags` — the newest tag by version order (the run
   of numbers in the name, so `v4.37.0` beats `v4.36.1` and `v4.9.0`). Branch
...
git tag -a v4.37.0 -m "SappiWhere 4.37.0"
git push origin v4.37.0
...
cat SHA256SUMS      # <64 hex chars>  magicalbeans-v4.37.0.tar.gz

# 3. Create the release for that tag and attach SHA256SUMS as an asset.
gh release create "$TAG" SHA256SUMS --title "SappiWhere 4.37.0" --notes-file -
```
