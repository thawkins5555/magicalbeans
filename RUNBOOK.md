...
4.39.0 that thread had no guard: a single transient database error killed it
permanently and silently, with nothing on any screen saying so. From 4.39.0 the
...
1. **Nodes tab, poller status line.** If it names an error, that is your
   answer; go to step 4. If the application is older than 4.39.0 it will say
...
**What it usually is.** One malformed datagram. Before 4.39.0 the receive
...
stopped", indistinguishable from an operator stopping it. From 4.39.0 the
...
recovered. This counter is new in 4.39.0; before it, this loss was completely
...
4.39.0 it loops to catch up within a per-tick budget and exposes how far behind
it is.
...
4.39.0 and worth recognising if you are running an older build:
...
5. **Reclaiming space.** Deleting rows does not shrink the file by itself.
   From 4.39.0 the databases run in incremental-vacuum mode and free pages are
...
**What `smtp_failing` means.** From 4.39.0 mail is sent by a queue thread with
...
4. **A rule that is too broad.** `trap_critical` matched every trap of any
   severity before 4.39.0, so a config-save trap opened a severity-2 "Critical
