...
|---|---|---|
| `test_nodepoll_e2e.py` | the poll chain end to end: scalars, IF-MIB tables, counters, flaps, reboots, a dark device, and the SNMP authentication events as transitions (repeat failures record one `auth_fail`; SNMP working again records exactly one `auth_ok`) | in-process `StubAgent` |
...
| `test_alert_operator_resolve.py` | an operator-resolved threshold alert (Nodes, NetPath and DHCP scope) does not re-open for the same breach run, but does after a clear plus a fresh breach; an engine auto-resolve is never mistaken for one; and a hand-resolved rollup parent covers the children it was hiding while its condition holds — three devices down, their packet-loss alerts stay shut until a device answers again, where acknowledging behaves the same as it always did | none — drives `AlertEngine` directly |
| `test_alerts_api.py` | the alert rows the web API hands the page: the device an alert resolves to (its own for a device alert, the parent switch for an interface one, none for anything outside Nodes or a device since removed) and the device mute that hangs off it — muting from an interface alert lands on the switch, shows in the mute list and on the Nodes row, and is refused with 403 to a read-only account | none — a real `Service` + `WebServer` over loopback |
