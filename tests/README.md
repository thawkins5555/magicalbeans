...
| `test_alert_engine_fixes.py` | the alert engine's 4.38.0 behaviour: cursors that advance only after a batch is applied (a poisoned occurrence is skipped, the rest still open), mail on a queue with a circuit breaker and the `smtp_failing` self-alert, `last_notified_ts` driving renotify from a per-tick sweep, trap and syslog identity keyed per source device and per message signature, the severity gate applied to traps, `auto_resolve_after_s`, `rules.notify`, `threshold_stale_s`, one keyed metric query per tick, and `upstream_id` rollup including the fan-out case | none — drives `AlertEngine` directly, with a `FakeMail` collector that can be told to raise |
