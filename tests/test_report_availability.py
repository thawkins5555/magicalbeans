...
set_status(nodesdb, did, "up")
event_at(nodesdb, did, "up", created + 60, "responding again")
...
for did in (maint_id, mute_id, plain_id):
    set_created(nodesdb, did, t0 - 3600)
    set_status(nodesdb, did, "down")
    event_at(nodesdb, did, "up", t0 - 1800, "responding again")
    # The SAME 4-hour outage, starting 6 hours in, still ongoing at t1, on
    # all three devices — the only difference between them is what
    # alertsdb says was covering the middle two hours of it.
    event_at(nodesdb, did, "down", t0 + 2 * 3600, "not responding")

down_start = t0 + 2 * 3600
mid_lo, mid_hi = down_start + 3600, down_start + 3 * 3600   # hours 1-3 of the outage

alertsdb.add_window("cutover", "devices", mid_lo, mid_hi, scope_device_ids=[maint_id])
# A still-active mute (until_ts in the future) is the only kind this
# module can see retroactively — see report.py's own module docstring on
# why an already-expired mute would not be — so the fixture reflects that
# rather than a mute that already lapsed.
...
check("maintenance device: 2 of the 6 down hours are excluded, not down",
     by_id[maint_id].down_s == 6 * 3600.0 - 2 * 3600.0
     and by_id[maint_id].maintenance_excluded_s == 2 * 3600.0,
     f"down_s={by_id[maint_id].down_s} excl={by_id[maint_id].maintenance_excluded_s}")
check("muted device: the same 2 hours are excluded via mute_excluded_s instead",
     by_id[mute_id].down_s == 6 * 3600.0 - 2 * 3600.0
     and by_id[mute_id].mute_excluded_s == 2 * 3600.0,
     f"down_s={by_id[mute_id].down_s} excl={by_id[mute_id].mute_excluded_s}")
check("plain device: nothing excluded, the full 6 hours count",
     by_id[plain_id].down_s == 6 * 3600.0
     and by_id[plain_id].maintenance_excluded_s == 0
     and by_id[plain_id].mute_excluded_s == 0,
     str(by_id[plain_id].down_s))
check("all three: still down at window end (never recovered)",
     all(by_id[i].still_down for i in (maint_id, mute_id, plain_id)))
